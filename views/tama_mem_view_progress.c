#include "tama_mem_view_progress.h"
...
static void tama_mem_view_progress_draw_progress(Canvas* canvas, float progress) {
...
static void
    tama_mem_view_progress_read_draw_callback(Canvas* canvas, SPIMemProgressViewModel* model) {
    canvas_draw_str_aligned(canvas, 64, 4, AlignCenter, AlignTop, "Reading dump");
    tama_mem_view_progress_draw_progress(canvas, model->progress);
    elements_button_left(canvas, "Cancel");
}

static void
    tama_mem_view_progress_draw_size_warning(Canvas* canvas, SPIMemProgressViewModel* model) {
...
static void
    tama_mem_view_progress_verify_draw_callback(Canvas* canvas, SPIMemProgressViewModel* model) {
    canvas_draw_str_aligned(canvas, 64, 2, AlignCenter, AlignTop, "Verifying dump");
    tama_mem_view_progress_draw_size_warning(canvas, model);
    tama_mem_view_progress_draw_progress(canvas, model->progress);
    elements_button_center(canvas, "Skip");
}

static void
    tama_mem_view_progress_write_draw_callback(Canvas* canvas, SPIMemProgressViewModel* model) {
...
static void tama_mem_view_progress_draw_callback(Canvas* canvas, void* context) {
    ...
    if(view_type == SPIMemProgressViewTypeRead) {
        tama_mem_view_progress_read_draw_callback(canvas, model);
    } else if(view_type == SPIMemProgressViewTypeVerify) {
        tama_mem_view_progress_verify_draw_callback(canvas, model);
    } else if(view_type == SPIMemProgressViewTypeWrite) {
        tama_mem_view_progress_write_draw_callback(canvas, model);
    }
}

static bool
    tama_mem_view_progress_read_write_input_callback(InputEvent* event, SPIMemProgressView* app) {
    bool success = false;
...
static bool
    tama_mem_view_progress_verify_input_callback(InputEvent* event, SPIMemProgressView* app) {
    bool success = false;
...
static bool tama_mem_view_progress_input_callback(InputEvent* event, void* context) {
    ...
    if(view_type == SPIMemProgressViewTypeRead) {
        success = tama_mem_view_progress_read_write_input_callback(event, app);
    } else if(view_type == SPIMemProgressViewTypeVerify) {
        success = tama_mem_view_progress_verify_input_callback(event, app);
    } else if(view_type == SPIMemProgressViewTypeWrite) {
        success = tama_mem_view_progress_read_write_input_callback(event, app);
    }
    return success;
...
SPIMemProgressView* tama_mem_view_progress_alloc() {
    ...
    view_set_draw_callback(app->view, tama_mem_view_progress_draw_callback);
    view_set_input_callback(app->view, tama_mem_view_progress_input_callback);
    tama_mem_view_progress_reset(app);
    return app;
}
...
static size_t tama_mem_view_progress_set_total_size(SPIMemProgressViewModel* model) {
...
void tama_mem_view_progress_inc_progress(SPIMemProgressView* app) {
    with_view_model(
        ...
        SPIMemProgressViewModel * model,
        {
            size_t total_size = tama_mem_view_progress_set_total_size(model);
