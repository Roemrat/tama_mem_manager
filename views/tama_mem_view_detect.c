#include "tama_mem_view_detect.h"
...
static void tama_mem_view_detect_draw_callback(Canvas* canvas, void* context) {
...
static void tama_mem_view_detect_enter_callback(void* context) {
    SPIMemDetectView* app = context;
    with_view_model(
        app->view, SPIMemDetectViewModel * model, { icon_animation_start(model->icon); }, false);
}

static void tama_mem_view_detect_exit_callback(void* context) {
    SPIMemDetectView* app = context;
    with_view_model(
...
SPIMemDetectView* tama_mem_view_detect_alloc() {
    ...
    with_view_model(
        ...
        false);
    view_set_draw_callback(app->view, tama_mem_view_detect_draw_callback);
    view_set_enter_callback(app->view, tama_mem_view_detect_enter_callback);
    view_set_exit_callback(app->view, tama_mem_view_detect_exit_callback);
    return app;
}
