...
const SPIMemChipVendorName tama_mem_chip_vendor_names[] = {
...
static const char* tama_mem_chip_search_vendor_name(SPIMemChipVendor vendor_enum) {
    const SPIMemChipVendorName* vendor = tama_mem_chip_vendor_names;
    while(vendor->vendor_enum != SPIMemChipVendorUnknown && vendor->vendor_enum != vendor_enum)
        vendor++;
    return vendor->vendor_name;
}

bool tama_mem_chip_find_all(SPIMemChip* chip_info, found_chips_t found_chips) {
...
void tama_mem_chip_copy_chip_info(SPIMemChip* dest, const SPIMemChip* src) {
    memcpy(dest, src, sizeof(SPIMemChip));
}

size_t tama_mem_chip_get_size(SPIMemChip* chip) {
    return (chip->size);
}

const char* tama_mem_chip_get_vendor_name(const SPIMemChip* chip) {
    return (tama_mem_chip_search_vendor_name(chip->vendor_enum));
}

const char* tama_mem_chip_get_vendor_name_by_enum(uint32_t vendor_enum) {
    return (tama_mem_chip_search_vendor_name(vendor_enum));
}

const char* tama_mem_chip_get_model_name(const SPIMemChip* chip) {
    return (chip->model_name);
}

uint8_t tama_mem_chip_get_vendor_id(SPIMemChip* chip) {
    return (chip->vendor_id);
}

uint8_t tama_mem_chip_get_type_id(SPIMemChip* chip) {
    return (chip->type_id);
}

uint8_t tama_mem_chip_get_capacity_id(SPIMemChip* chip) {
    return (chip->capacity_id);
}

SPIMemChipWriteMode tama_mem_chip_get_write_mode(SPIMemChip* chip) {
    return (chip->write_mode);
}

size_t tama_mem_chip_get_page_size(SPIMemChip* chip) {
    return (chip->page_size);
}

uint32_t tama_mem_chip_get_vendor_enum(const SPIMemChip* chip) {
