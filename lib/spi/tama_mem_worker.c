...
static int32_t tama_mem_worker_thread(void* thread_context);
...
static int32_t tama_mem_worker_thread(void* thread_context) {
    ...
    while(true) {
        ...
        if(flags != (unsigned)FuriFlagErrorTimeout) {
            ...
            if(tama_mem_worker_modes[worker->mode_index].process) {
                tama_mem_worker_modes[worker->mode_index].process(worker);
            }
            worker->mode_index = SPIMemWorkerModeIdle;
        }
    }
