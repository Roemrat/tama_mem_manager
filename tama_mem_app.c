...
static bool tama_mem_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
    SPIMemApp* app = context;
    return scene_manager_handle_custom_event(app->scene_manager, event);
}

static bool tama_mem_back_event_callback(void* context) {
    furi_assert(context);
    SPIMemApp* app = context;
    return scene_manager_handle_back_event(app->scene_manager);
}

SPIMemApp* tama_mem_alloc(void) {
    ...
    instance->scene_manager = scene_manager_alloc(&tama_mem_scene_handlers, instance);
    instance->submenu = submenu_alloc();
    instance->dialog_ex = dialog_ex_alloc();
    instance->popup = popup_alloc();
    instance->worker = tama_mem_worker_alloc();
    instance->dialogs = furi_record_open(RECORD_DIALOGS);
    instance->storage = furi_record_open(RECORD_STORAGE);
    instance->widget = widget_alloc();
    instance->chip_info = malloc(sizeof(SPIMemChip));
    found_chips_init(instance->found_chips);
    instance->view_progress = tama_mem_view_progress_alloc();
    instance->view_detect = tama_mem_view_detect_alloc();
    ...
    view_dispatcher_set_custom_event_callback(
        instance->view_dispatcher, tama_mem_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(
        instance->view_dispatcher, tama_mem_back_event_callback);
    ...
    view_dispatcher_add_view(
        ...
        SPIMemViewProgress,
        tama_mem_view_progress_get_view(instance->view_progress));
    view_dispatcher_add_view(
        instance->view_dispatcher,
        SPIMemViewDetect,
        tama_mem_view_detect_get_view(instance->view_detect));
    view_dispatcher_add_view(
...
void tama_mem_free(SPIMemApp* instance) {
    ...
    tama_mem_view_progress_free(instance->view_progress);
    tama_mem_view_detect_free(instance->view_detect);
    ...
    tama_mem_worker_free(instance->worker);
...
int32_t tama_mem_app(void* p) {
    UNUSED(p);
    SPIMemApp* instance = tama_mem_alloc();
    view_dispatcher_run(instance->view_dispatcher);
    tama_mem_free(instance);
    return 0;
}
