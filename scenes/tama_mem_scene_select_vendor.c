...
static void tama_mem_scene_select_vendor_submenu_callback(void* context, uint32_t index) {
    SPIMemApp* app = context;
    app->chip_vendor_enum = index;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void tama_mem_scene_select_vendor_sort_vendors(SPIMemApp* app, vendors_t vendors_arr) {
    for(size_t index = 0; index < found_chips_size(app->found_chips); index++) {
        vendors_push_back(
            vendors_arr, tama_mem_chip_get_vendor_enum(*found_chips_get(app->found_chips, index)));
    }
    vendors_uniq(vendors_arr);
}

void tama_mem_scene_select_vendor_on_enter(void* context) {
    SPIMemApp* app = context;
    vendors_t vendors_arr;
    vendors_init(vendors_arr);
    tama_mem_scene_select_vendor_sort_vendors(app, vendors_arr);
    size_t vendors_arr_size = vendors_size(vendors_arr);
    if(vendors_arr_size == 1)
        tama_mem_scene_select_vendor_submenu_callback(context, *vendors_get(vendors_arr, 0));
    for(size_t index = 0; index < vendors_arr_size; index++) {
        uint32_t vendor_enum = *vendors_get(vendors_arr, index);
        submenu_add_item(
            app->submenu,
            tama_mem_chip_get_vendor_name_by_enum(vendor_enum),
            vendor_enum,
            tama_mem_scene_select_vendor_submenu_callback,
            app);
    }
...
static void tama_mem_scene_select_vendor_set_previous_scene(SPIMemApp* app) {
...
bool tama_mem_scene_select_vendor_on_event(void* context, SceneManagerEvent event) {
    SPIMemApp* app = context;
    bool success = false;
    if(event.type == SceneManagerEventTypeBack) {
        success = true;
        tama_mem_scene_select_vendor_set_previous_scene(app);
...
void tama_mem_scene_select_vendor_on_exit(void* context) {
    SPIMemApp* app = context;
