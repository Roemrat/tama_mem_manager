...
static void
    tama_mem_scene_erase_widget_callback(GuiButtonType result, InputType type, void* context) {
...
static void tama_mem_scene_erase_callback(void* context, SPIMemCustomEventWorker event) {
    SPIMemApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, event);
}

void tama_mem_scene_erase_on_enter(void* context) {
    SPIMemApp* app = context;
    widget_add_button_element(
        app->widget, GuiButtonTypeLeft, "Cancel", tama_mem_scene_erase_widget_callback, app);
    ...
    tama_mem_worker_erase_start(app->chip_info, app->worker, tama_mem_scene_erase_callback, app);
}

static void tama_mem_scene_erase_set_previous_scene(SPIMemApp* app) {
...
static void tama_mem_scene_erase_set_next_scene(SPIMemApp* app) {
...
bool tama_mem_scene_erase_on_event(void* context, SceneManagerEvent event) {
    SPIMemApp* app = context;
    bool success = false;
    if(event.type == SceneManagerEventTypeBack) {
        success = true;
        tama_mem_scene_erase_set_previous_scene(app);
    } else if(event.type == SceneManagerEventTypeCustom) {
        success = true;
        if(event.event == GuiButtonTypeLeft) {
            scene_manager_previous_scene(app->scene_manager);
        } else if(event.event == SPIMemCustomEventWorkerDone) {
            tama_mem_scene_erase_set_next_scene(app);
...
void tama_mem_scene_erase_on_exit(void* context) {
    SPIMemApp* app = context;
