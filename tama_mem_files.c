...
bool tama_mem_file_select(SPIMemApp* app) {
    ...
    dialog_file_browser_set_basic_options(&browser_options, tama_mem_FILE_EXTENSION, &I_Dip8_10px);
...
bool tama_mem_file_create_open(SPIMemApp* app) {
    ...
    do {
        if(furi_string_end_with(app->file_path, tama_mem_FILE_EXTENSION)) {
            if(!tama_mem_file_delete(app)) break;
            size_t filename_start = furi_string_search_rchar(app->file_path, '/');
            furi_string_left(app->file_path, filename_start);
        }
        furi_string_cat_printf(app->file_path, "/%s%s", app->text_buffer, tama_mem_FILE_EXTENSION);
